...
void CacheObliviousTranspose(int i, int j, int dimension) {
  ...
  if (dimension <= 64) {
    ...
    CacheObliviousTranspose(i, j, midpoint);
    // the upper right quadrant
    CacheObliviousTranspose(i + midpoint, j, midpoint);
    // the lower left quadrant
    CacheObliviousTranspose(i, j + midpoint, midpoint);
    // the lower right quadrant
    CacheObliviousTranspose(i + midpoint, j + midpoint, midpoint);
  }
}
